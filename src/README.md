Perl {{ spec.version }} container image
=========================

This container image includes Perl {{ spec.version }} as an [S2I](https://github.com/openshift/source-to-image) base image for your Perl {{ spec.version }} applications.
...
Perl {{ spec.version }} available as container is a base platform for
...
In this example, we will assume that you are using the `{{ spec.ubi }}/perl-{{ spec.short }}` image, available via `perl:{{ spec.version }}` imagestream tag in Openshift.
To build a simple [perl-sample-app](https://github.com/sclorg/dancer-ex.git) application in Openshift:

```
oc new-app perl:{{ spec.version }}~https://github.com/sclorg/dancer-ex.git
```
...
```
podman pull {{ spec.ubi }}/perl-{{ spec.short }}
```

An {{ spec.ubi }} image `{{ spec.ubi }}/perl-{{ spec.short }}` is used in this example.
